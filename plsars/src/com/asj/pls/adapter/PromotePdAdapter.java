...
public class PromotePdAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			hold.cart = (TextView) convertView.findViewById(R.id.promote_pd_cart);
...
//			}
//		});
		hold.cart.setOnClickListener(new OnClickListener(){

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				String price = pdList.get(position).getPrice().substring(1, pdList.get(position).getPrice().length());
	...
	static class Holder {
		private TextView pdName, price, tag, cart;
	...
	private void setAnim(final View v, int[] start_location, final String price) {
		...
		PromoteActivity.cartNum.getLocationInWindow(end_location);// shopCart是那个购物车
		...
		set.setAnimationListener(new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				Model.cartNum += 1;
				PromoteActivity.cartNum.setText(String.valueOf(Model.cartNum));
				Model.cartPrice = Model.cartPrice.add(new BigDecimal(price));
				PromoteActivity.cartPriceText.setText("￥"+Model.cartPrice.toString());
			}
		});
